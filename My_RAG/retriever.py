...
class VectorRetriever:
    ...
    def _index_documents(self) -> np.ndarray:
        ...
        try:
            ...
            print(f"[SUCCESS] Vector model '{self.embedding_model}' indexed {len(embeddings)} chunks.")
            return embeddings
...
class RemoteFlagReranker:
    ...
    def compute_score(self, pairs: List[List[str]], max_length: int = 1024) -> np.ndarray:
        MAX_PAIRS_PER_CALL = 32
        all_scores: List[float] = []

        for i in range(0, len(pairs), MAX_PAIRS_PER_CALL):
            batch_pairs = pairs[i:i + MAX_PAIRS_PER_CALL]
            payload = {"pairs": [{"text1": a[:max_length], "text2": b[:max_length]} for a, b in batch_pairs]}
            ...
            try:
                ...
                if len(scores) != len(batch_pairs):
                    print("[Reranker API Error] scores length mismatch. Returning zeros for this batch.")
                    all_scores.extend([0.0] * len(batch_pairs))
                    continue
                ...
                print(f"[Reranker Connection Error] {e}. Returning zero scores for batch.")
                all_scores.extend([0.0] * len(batch_pairs))
...
class EnsembleRetriever:
    def __init__(self, chunks: List[dict], language: str, client: Client):
        ...
        self.vector_retriever = VectorRetriever(chunks, language, client, embedding_model="qwen3-embedding:0.6b")
    ...
    def retrieve(self, query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        if merged_results:
            pairs_to_rerank = [[query, item["chunk"]["page_content"]] for item in merged_results]
...
def create_retriever(chunks: List[dict], language: str, client= None) -> EnsembleRetriever:
