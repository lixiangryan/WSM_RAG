...
class VectorRetriever:
    ...
    def _index_documents(self):
        ...
        try:
            ...
            # 2. 加入成功的確認訊息
            print(
                f"[SUCCESS] Vector model '{self.model_name}' indexed {len(embeddings)} chunks."
            )
            return embeddings
        ...
        for i in range(0, len(pairs), MAX_PAIRS_PER_CALL):
            ...
            try:
                ...
                if resp.status_code != 200:
                    print(
                        f"[Reranker API Error] Request failed ({resp.status_code}): {resp.text}"
                    )
                ...
                # 連線層級失敗，回傳零分
                print(
                    f"[Reranker Connection Error] Failed to connect to API: {e}. Returning zero scores for batch."
                )
                all_scores.extend(np.zeros(len(batch_pairs)).tolist())
